...
const {exec} = require('ait-process');
  ...
  try {
    ...
    log.info(
      `The Chromedriver install script cannot be found at '${BUILD_PATH}'. ` +
        `Building armor-chromedriver package`
    );
    const npmCommand = process.platform === 'win32' ? 'npm.cmd' : 'npm';
    try {
      await exec(npmCommand, ['run', 'build'], {logger: log, cwd: __dirname});
    } catch (e) {
      throw new Error(`armor-chromedriver package cannot be built: ${util.inspect(e)}`);
    }
  }

  // check if we should skip install
  if (
    !_.isEmpty(process.env.ARMOR_SKIP_CHROMEDRIVER_INSTALL) ||
    !_.isEmpty(process.env.npm_config_chromedriver_skip_install)
  ) {
    log.warn(
      `'ARMOR_SKIP_CHROMEDRIVER_INSTALL' environment variable is set; skipping Chromedriver installation.`
    );
  ...
  try {
    ...
    log.error(
      `Downloading Chromedriver can be skipped by setting the` +
        `'ARMOR_SKIP_CHROMEDRIVER_INSTALL' environment variable.`
    );
    process.exit(1);
  }
}
