...
class Chromedriver extends events_1.default.EventEmitter {
  ...
  constructor(args = {}) {
    ...
    const {
      ...
      executable,
      executableDir = (0, utils_1.getChromedriverDir)(),
      bundleId,
      mappingPath,
      cmdArgs,
      adb,
    ...
    this._log = armor_support_1.logger.getLogger((0, utils_1.generateLogPrefix)(this));
    this.proxyHost = host;
    this.proxyPort = port;
    this.adb = adb;
    ...
    this.jwproxy = new armor_base_driver_1.JWProxy({
    ...
    this.desiredProtocol = armor_base_driver_1.PROTOCOLS.MJSONWP;
  ...
  async getDriversMapping() {
    ...
    if (this.mappingPath) {
      ...
      if (!(await armor_support_1.fs.exists(this.mappingPath))) {
        this.log.warn(`No file found at '${this.mappingPath}'`);
        this.log.info('Defaulting to the static Chromedriver->Chrome mapping');
      } else {
        try {
          mapping = JSON.parse(await armor_support_1.fs.readFile(this.mappingPath, 'utf8'));
        } catch (e) {
  ...
  async getChromedrivers(mapping) {
    ...
    const executables = await armor_support_1.fs.glob('*', {
      cwd: this.executableDir,
      nodir: true,
      absolute: true,
    });
    this.log.debug(
      `Found ${armor_support_1.util.pluralize('executable', executables.length, true)} ` +
        `in '${this.executableDir}'`,
    );
    const cds = (
      await (0, ait_async_1.asyncmap)(executables, async (executable) => {
        /**
         * @param {{message: string, stdout?: string, stderr?: string}} opts
         */
        const logError = ({message, stdout, stderr}) => {
          let errMsg =
            `Cannot retrieve version number from '${path_1.default.basename(
              executable,
            )}' Chromedriver binary. ` +
        ...
        let stderr;
        try {
          ({stdout, stderr} = await (0, ait_process_1.exec)(executable, ['--version'], {
            timeout: CD_VERSION_TIMEOUT,
          }));
        } catch (e) {
          const err = /** @type {ExecError} */ (e);
          if (
  ...
  async updateDriversMapping(newMapping) {
    ...
    if (await armor_support_1.fs.exists(this.mappingPath)) {
      try {
        await armor_support_1.fs.writeFile(
          this.mappingPath,
  ...
  async getCompatibleChromedriver() {
    ...
    if (!lodash_1.default.isEmpty(mapping)) {
      this.log.debug(
        `The most recent known Chrome version: ${lodash_1.default.values(mapping)[0]}`,
      );
    }
    ...
    do {
      ...
      if (!lodash_1.default.isEmpty(missingVersions)) {
        this.log.info(
          `Found ${armor_support_1.util.pluralize(
            'Chromedriver',
            lodash_1.default.size(missingVersions),
            true,
          )}, ` +
            `which ${
              lodash_1.default.size(missingVersions) === 1 ? 'is' : 'are'
      ...
      this.log.debug(
        `Found ${armor_support_1.util.pluralize('executable', matchingDrivers.length, true)} ` +
  ...
  async initChromedriverPath() {
    ...
    if (!(await armor_support_1.fs.exists(chromedriver))) {
      throw new Error(
  ...
  syncProtocol() {
    ...
    if (!coercedVersion || coercedVersion.major < MIN_CD_VERSION_WITH_W3C_SUPPORT) {
      this.log.debug(
        `The WebDriver v. ${this.driverVersion} does not fully support ${armor_base_driver_1.PROTOCOLS.W3C} protocol. ` +
          `Defaulting to ${armor_base_driver_1.PROTOCOLS.MJSONWP}`,
      );
      return;
    }
    // Check only chromeOptions for now.
    const chromeOptions = (0, protocol_helpers_1.getCapValue)(
      this.capabilities,
      'chromeOptions',
      {},
    );
    if (chromeOptions.w3c === false) {
      this.log.info(
        `The WebDriver v. ${this.driverVersion} supports ${armor_base_driver_1.PROTOCOLS.W3C} protocol, ` +
          `but ${armor_base_driver_1.PROTOCOLS.MJSONWP} one has been explicitly requested`,
      );
      return;
    }
    this.desiredProtocol = armor_base_driver_1.PROTOCOLS.W3C;
  ...
  async start(caps, emitStartingState = true) {
    ...
    this.capabilities.loggingPrefs = lodash_1.default.cloneDeep(
      (0, protocol_helpers_1.getCapValue)(caps, 'loggingPrefs', {}),
    );
    ...
    try {
      ...
      this.proc = new ait_process_1.SubProcess(chromedriverPath, args);
      processIsAlive = true;
      // handle log output
      this.proc.on('output', (stdout, stderr) => {
        ...
        //   DevTools response: {
        //      "Android-Package": "io.armor.sampleapp",
  ...
  async waitForOnline() {
    ...
    await (0, ait_async_1.retryInterval)(20, 200, async () => {
  ...
  async killAll() {
    let cmd = armor_support_1.system.isWindows()
