...
class Chromedriver extends events_1.default.EventEmitter {
    ...
    async getChromedrivers(mapping) {
        ...
        this.log.debug(`Found ${armor_support_1.util.pluralize('executable', executables.length, true)} ` +
            `in '${this.executableDir}'`);
        const cds = (await (0, ait_async_1.asyncmap)(executables, async (executable) => {
            /**
    ...
    async updateDriversMapping(newMapping) {
        ...
        if (await armor_support_1.fs.exists(this.mappingPath)) {
            try {
                await armor_support_1.fs.writeFile(this.mappingPath, JSON.stringify(newMapping, null, 2), 'utf8');
    ...
    async getCompatibleChromedriver() {
        ...
        do {
            ...
            if (!lodash_1.default.isEmpty(missingVersions)) {
                this.log.info(`Found ${armor_support_1.util.pluralize('Chromedriver', lodash_1.default.size(missingVersions), true)}, ` +
            ...
            this.log.debug(`Found ${armor_support_1.util.pluralize('executable', matchingDrivers.length, true)} ` +
    ...
    syncProtocol() {
        ...
        if (!coercedVersion || coercedVersion.major < MIN_CD_VERSION_WITH_W3C_SUPPORT) {
            this.log.debug(`The WebDriver v. ${this.driverVersion} does not fully support ${armor_base_driver_1.PROTOCOLS.W3C} protocol. ` +
        ...
        if (chromeOptions.w3c === false) {
            this.log.info(`The WebDriver v. ${this.driverVersion} supports ${armor_base_driver_1.PROTOCOLS.W3C} protocol, ` +
    ...
    async startSession() {
        const sessionCaps = this.desiredProtocol === armor_base_driver_1.PROTOCOLS.W3C
